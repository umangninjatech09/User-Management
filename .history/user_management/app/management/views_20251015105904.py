...
class UserUpdateView(APIView):
    ...
    def put(self, request): 
        ...
        try:
            # Fetch the user instance based on the token
            user = User.objects.get(pk=user_id, is_deleted=False)
        except User.DoesNotExist:
            # This should rarely happen if IsAuthenticated works, but good for safety
            return Response({'error': 'Authenticated user not found'}, status=status.HTTP_404_NOT_FOUND)

        # Logic to prevent changing the email address
        ...
        # Use partial=True for fields that might be missing (common in PUT/PATCH)
