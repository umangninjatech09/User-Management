...
class RequestOTPView(APIView):
    def post(self, request):
        ...
        # 💡 FIX: Use email__iexact for case-insensitive matching.
        # This will match 'USER@EXAMPLE.COM' with 'user@example.com' in the database.
        matching_users = User.objects.filter(email__iexact=email, is_deleted=False)
        
        if not matching_users.exists():
            return Response({"error": "User not found"}, status=status.HTTP_404_NOT_FOUND)

        # ⚠️ WARNING: Since email is not unique in the database, 
        # using .first() arbitrarily selects one account.
        user = matching_users.first()
        
        # Current logic sends OTP for the first user found:
