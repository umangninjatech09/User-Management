...
class UserDeleteView(APIView):
    ...
    # 💡 IMPORTANT: Remove 'pk' from the method signature
    def delete(self, request): 
        # Use the ID from the authenticated user's token
        user_id = request.user.pk
        
        try:
            # Fetch the user instance based on the token ID
            user = User.objects.get(pk=user_id, is_deleted=False)
        except User.DoesNotExist:
            # If the user exists in the token but not the database (unlikely but safe)
            return Response({'error': 'Authenticated user not found'}, status=status.HTTP_404_NOT_FOUND)
        ...
        # Invalidate the session/token (optional, but highly recommended)
        # request.auth.delete() # If using Token or JWT auth with invalidation
