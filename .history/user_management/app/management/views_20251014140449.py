...
class UserUpdateView(APIView):
    ...
    def put(self, request, pk):
        ...
        # -----------------------------------------------------
        # 1. Check if 'email' is present in the request data
        if 'email' in request.data:
            # 2. Compare the requested email with the existing email
            if request.data['email'] != user.email:
                return Response(
                    {'error': 'Email address cannot be changed through this endpoint.'}, 
