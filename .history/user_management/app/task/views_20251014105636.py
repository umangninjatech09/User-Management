...
class TaskListCreateView(APIView):
    ...
    def post(self, request):
        ...
        # 💡 Step 1: Create a mutable copy of the incoming data
        mutable_data = request.data.copy()
        
        # 💡 Step 2: Remove 'assigned_to' if present, ensuring client cannot override it
        mutable_data.pop('assigned_to', None)

        serializer = TaskSerializer(data=mutable_data)

        if serializer.is_valid():
            # 💡 Step 3: Automatically set assigned_to to the authenticated user
            serializer.save(
                assigned_to=authenticated_user,  # <-- NEW: Sets the user from the token
